...
*}
	...
	<script src="{$js_dir}/jquery-1.4.4.min.js"></script>
	<script src="{$js_dir}/jquery-ui-1.8.7.min.js"></script>
	<script src="{$js_dir}/jquery.ui.addresspicker.js"></script>
	...
	<script>
			$(function() {
				var addresspickerMap = $jq14( "#address1" ).addresspicker({
					regionBias: "fr",
				  elements: {
				    map:      "#map",
				    locality: '#city',
	...
	<div id="map" style="min-height:190px;"></div>

<div class="message_google_map_acceuil">
	<p>
		<input type="submit" name="geolocalisation" id="geolocalisation" value="Geolocalisation" class="button" />
	</p>
	<p>
		<span id="maposition" name="maposition"><span>
	</p>
	</div>
...
{/strip}
	...
	{* SCRIPT GEOLOC *}
	<script src="{$js_dir}/geo.js"></script>
{literal}
<script>
	$("#geolocalisation").live("click", function() {
...
if(geo_position_js.init()){
	...
	initializegeocoding (position.coords.latitude,position.coords.longitude);
	codeLatLng(position.coords.latitude, position.coords.longitude, "maposition");
 ...
 var geocoder;
  ...
  var marker;
  function initializegeocoding(latitude,longitude) {
    ...
    map = new google.maps.Map(document.getElementById("map"), mapOptions);
	...
	}   function codeLatLng(lat, lng, maposition) {
	  geocoder = new google.maps.Geocoder();
	  ...
	  geocoder.geocode({'latLng': latlng}, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
        	...
        	document.getElementById(maposition).innerHTML = document.getElementById(maposition).innerHTML+results[0].formatted_address;
			document.getElementById("address1").value = results[0].formatted_address;
			var postcode=results[2].formatted_address.split(reg);
			document.getElementById("postcode").value = postcode[0];
			var city=results[4].formatted_address.split(reg);
			document.getElementById("city").value = city[0];
			for(var i = 0 ,l = document.getElementById("id_state").options.length; i< l;i++ ){
					if(document.getElementById('id_state')[i].innerHTML == postcode[0]){
						document.getElementById("id_state").value = document.getElementById("id_state").options[i].value;
					}
			}}
        else {
            alert("Geocoder failed due to: " + status);
...
$('#postcode').change(function() {
			for(var i = 0 ,l = document.getElementById("id_state").options.length; i< l;i++ ){
					if(document.getElementById('id_state')[i].innerHTML == document.getElementById('postcode').value){
						document.getElementById("id_state").value = document.getElementById("id_state").options[i].value;
					}
			}
	});
